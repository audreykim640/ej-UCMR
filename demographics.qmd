...
ggplot(full, aes(x = MedIncome, y = ContamLevel)) +
  geom_point(alpha = 1) +
  geom_smooth(se = FALSE, formula = y~x, color = "goldenrod") +
  facet_grid(~ Contaminant, scales = "free") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = -90, vjust = 0.4))
```
...
germanium <- full %>% 
  ...
  mutate(LogLevel = log10(ContamLevel))
...
glm(LogLevel ~ Prop * RaceEth, data = germanium)
```
...
#### Manganese levels vs Race/ethnicity served by PWS proportions 

```{r}
manganese <- full %>% 
  filter(Contaminant == "manganese")

ggplot(manganese, aes(x = Prop, y = ContamLevel)) +
  geom_point(alpha = 1) +
  geom_smooth(method = "glm", se = FALSE,
       method.args = list(family = "Gamma"), color = "goldenrod") +
  facet_grid(Contaminant ~ RaceEth, switch = "both", scale = "free") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = -90, vjust = 0.4))

manganese_model <- glm(ContamLevel ~ Prop * RaceEth, data = manganese, family = Gamma)
manganese_model2 <- glm(ContamLevel ~ Prop * RaceEth, data = manganese)

rcompanion::compareGLM(manganese_model, manganese_model2)

plot(manganese_model)
...
I'm trying to figure out which statistical tests are valid and what I should be using.
Um check assuptions for ANOVA (violin) for means and see what can be done for linear
regression. xx :)
