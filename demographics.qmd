...
ggplot(full, aes(x = MedIncome, y = ContamLevel)) +
  geom_point(alpha = 1) +
  geom_smooth(se = FALSE, formula = y~x, color = "goldenrod") +
  facet_grid(~ Contaminant, scales = "free_y") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = -90, vjust = 0.4))
```
...
germanium_model <- lm(ContamLevel ~ Prop * RaceEth, data = germanium)
...
ggplot(germ_residuals, aes(sample = residual)) +
  geom_qq_line() +
  geom_qq() +
  facet_wrap(~ RaceEth)

ggplot(germ_residuals, aes(x = ContamLevel_hat, y = residual)) +
  geom_point() +
  facet_wrap(~ RaceEth)
# +
#   geom_smooth(se = FALSE, formula = y~x, color = "goldenrod") +
