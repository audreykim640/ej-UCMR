...
```{r}
watershapes <- st_read("data/temm.gpkg") # https://community.rstudio.com/t/how-to-open-a-geopackage-file/41027
...
```{r}
UCMRIndigCodes <- UCMRall %>% 
  filter(PWSName %in% c("Pala North", "Pauma", "Pechanga", "Redding Rancheria", 
                        "Viejas Community System")) %>% 
  select(PWSID) %>% 
  unique() %>% 
  pull() %>% 
  map(function (x) { paste0("0", x)})

indigwatershapes <- watershapes %>% 
  filter(pwsid %in% UCMRIndigCodes) %>% 
  rename("PWSID" = "pwsid",
         "WATER_SY_1" = "pws_name",
         "geometry" = "geom") %>% 
...
CAwatershapes <- bind_rows(indigwatershapes, govwatershapes)
